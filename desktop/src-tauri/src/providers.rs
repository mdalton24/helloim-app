...
fn caveat_for(kind: &str) -> &'static str {
    match kind {
        /* THE LAST SENTENCE WAS ADDED 2026-08-31, THE HOUR THE NATIVE ENGINE'S
           GATE OPENED, AND IT IS THE PRICE OF THAT CHANGE SAID OUT LOUD.
           A local brain no longer runs through Claude Code — it runs through
           NameOS's own loop, which is what removes the download, the terminal
           and the sign-in. It is also what removes the tools: that engine has no
           Read, Write, Edit or Bash, and no connectors. **A capability that
           disappears without a word is worse than one that was never offered**,
           and this string is the one place the app describes what this kind of
           brain is. It says what you gain and what you give up, in that order,
           in the customer's own terms — "your files" rather than "tool use". */
        "local" => {
            "Runs on your own machine. Free and private, with nothing to install \
             and nobody to sign in to. Slower than Claude, and smaller models get \
             details confidently wrong — good for drafts and summaries, not for \
             work you'd act on unchecked. It talks and listens only: it cannot \
             open, change or run anything in your folder."
        }
        // RESTORED 2026-08-29, WORD FOR WORD FROM `3a328ff`, because it carried
        // a ruling: "a company that tells you a competitor's option might cost
        // you more is the company you believe when it says a connection is
        // green." Softening the one sentence that costs us something would
        // spend the only asset the honesty system is building. **Do not reword
        // it.**
        //
        // It is returned even while OPENAI_ENABLED is false, and that is not an
        // oversight. A caveat is a property of a KIND, not of whether a screen
        // is currently offering it; making it conditional would mean the gate
        // flipping one day and the warning not coming with it. With the gate
        // shut no row of this kind is SELECTABLE — `normalized()` retires the
        // row, and RETIRED_NOTE plus a lone Remove button is all it offers.
        //
        // THIS USED TO SAY "so nobody sees this string" AND THAT WAS WRONG —
        // corrected 2026-08-29, caught on real Windows by planting an
        // `openai-compatible` row into `providers.json` and looking at what
        // rendered. The caveat IS displayed on the retired row, beside the
        // retirement note. Harmless today, because the row offers nothing to
        // press. **The reason it is corrected rather than left is that a
        // comment asserting nobody can see a string is exactly the kind of
        // claim a future change leans on** — someone widens what a retired row
        // shows, trusts this line, and a sentence written for a screen nobody
        // was meant to reach is suddenly load-bearing.
        "openai-compatible" => {
            "A different company's model and servers. Your conversation goes \
             to them through helloim.ai's built-in translator, and it may cost \
             more per message than Claude because nothing is cached."
        }
        // `anthropic-compatible` is NOT restored. It was removed the same day
        // and Mark's instruction tonight named two choices — Claude and
        // OpenAI. A kind nobody asked for is a kind nobody tested.
        //
        // A retired kind gets NO caveat, deliberately: a caveat is the cost of
        // a choice, said where the choosing happens, and there is no choice to
        // make on a row that cannot be selected. Its `migration_note` carries
        // the whole message instead — see RETIRED_NOTE.
        _ => "",
    }
}
...
mod tests {
    ...
    fn the_users_anthropic_key_is_never_forwarded() {
        let env = provider_env(&row("local"), "http://10.0.0.51:11434", "their-key".into());
        assert_eq!(
    ...
    fn caveats_are_derived_and_honest() {
        assert!(caveat_for("local").contains("confidently wrong"));
        ...
        assert!(caveat_for("openai-compatible").contains("nothing is cached"),
            "the cost sentence ships as written — do not reword it");
    ...
    fn opening_the_gate_makes_the_row_real_rather_than_merely_present() {
        ...
        with_openai_enabled(|| {
            ...
            assert!(got.caveat.contains("nothing is cached"), "the cost warning travels with it");
        });
    }
    ...
    fn a_lookalike_hostname_is_not_this_machine() {
        ...
        assert!(!is_loopback("http://10.0.0.51:11434"));
    }
