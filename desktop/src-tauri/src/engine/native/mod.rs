...
///
/// **STRICT-COMPATIBLE, ON PURPOSE, EVEN THOUGH ONLY ONE WIRE READS `strict`
/// TODAY.** `parameters` must carry `"additionalProperties": false` and list
/// EVERY property — including the ones that read as optional — in
/// `"required"`, with an optional one typed `["type", "null"]` instead of
/// simply absent from `required`. That is OpenAI's own strict-mode contract
/// (confirmed against the live schema-validation error, not assumed from a
/// page — a definition missing either one is rejected outright with the
/// missing constraint named). Building every tool this way from the start
/// means a wire that wants `strict: true` never has to renegotiate the
/// schema later, and a wire that ignores strictness (Ollama's own tool
/// calling, when that lands) loses nothing by the schema being stricter than
/// it needs.
...
fn drive_with_timeout(
    ...
    let completion = 'rounds: loop {
        ...
        for call_req in completion.tool_calls.clone() {
            ...
            let result = if call_req.name == "OpenUrl" {
...
mod tests {
    ...
    fn a_turn_emits_the_shapes_the_window_actually_parses() {
        ...
        assert_eq!(sys.len(), 1, "one opening line: {ev:?}");
