...
pub(crate) fn confine(root: &Path, requested: &str) -> Result<PathBuf, String> {
    ...
    let resolved = match real_ancestor {
        ...
        // Nothing on the path exists at all yet, not even the parent
        // directory — the lexical form is all there is to check. A tool
        // asking to create a whole new directory tree gets the same
        // containment check as one asking for a file; it just runs on the
        // string rather than on a canonicalized ancestor.
        None => lexical,
    };
