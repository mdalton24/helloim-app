...
//
// The user, 2026-08-31: "we need to make each of the providers stand alone without
...
// The OpenAI translator. **REWIRED 2026-08-29 AND NOW REACHABLE** — the user took
...
pub(crate) struct Session {
    ...
    /// The security reviewer's race 2 of 2 on the deadlock fix.** `cancel_running_turn`
    ...
    /// **SERIALIZES CONCURRENT CANCELLERS — added 2026-09-04, the security reviewer's
...
/// `the safety spec` room amendment 2.** `TurnSink::confirm` needs to
...
/// never run: there is no Tauri window to click a button in here. What the release reviewer
...
/// RED AND READS AS A FAILURE.** the user's first message to a local brain came
...
/// The release reviewer, 2026-08-31.** The marker was assumed to be its own whole line, with
/// the `{"model":...}` detail on a SEPARATE line beneath it, because that is
/// how it read in the screenshot the user sent. It is not two lines. The release reviewer spawned
...
/// `claude` build may still split it, and the release reviewer could not confirm which
/// version ships on the user's own machine (his account, not hers) — so a bare
...
struct Preflight {
    ...
    /// Is the binary actually SIGNED IN — the user, 2026-08-27: "bella was setup,
...
///
/// **THE RESIDUAL OF W5 -- found 2026-08-29.** W5 changed the wizard and
...
fn preflight() -> Preflight {
    ...
    let mut cmd = Command::new(&path);
    cmd.arg("--version");
    /* PLUGINS, THE WAY CLAUDE CODE ALREADY TAKES THEM -- the user, 2026-08-26:
    ...
    hide_console(&mut cmd);
    // BOUNDED, NOT `cmd.output()` -- found 2026-08-29 (the W5 residual). See
...
/// window-close bug (NameOS.exe alive 187s after every window was gone,
/// found on an earlier internal build) would have looked identical to a correct fix if nothing external to
...
/// way there.
///
/// **THAT FIX OPENED TWO RACES OF ITS OWN — found by the security reviewer, 2026-09-04,
...
///    Exiting mid-kill is the same orphan bug back under a race. Closed by
...
/// One row in the History tab — the designer's IA spec, "Same layout as SupportAssist's
...
/// The user, 2026-09-05, running the shipped app — as "history doesn't even
...
/// FLAGGED FOR a front-end reviewer, NOT FIXED HERE.** `ui/index.html`'s `iaRenderHistory`
...
/// brain" — and that is the front-end reviewer's screen, not this file's.
...
fn send(
    ...
    /* THE TRUST BOUNDARY, AND IT IS HERE RATHER THAN IN THE WINDOW ON PURPOSE.
       ...
       precisely because the security reviewer proved the unframed version exploitable.
    ...
    // THE FOLDER-FEATURE PROMISE VS "ALLOW EVERYTHING" -- a later design review,
    // The security reviewer/the outside reviewer's finding, 2026-09-06/07: on `ClaudeCodeEngine` with
    ...
    /* `ClaudeCodeEngine` NEVER NEEDS THIS: it spawns a real `claude.exe` in
       ...
       every native turn. Found by the front-end reviewer, 2026-09-01, tracing the write side;
...
/// nothing reads yet -- the front-end reviewer/the visual designer's own layout for it is still unbuilt) --
...
impl engine::TurnSink for AppSink {
    ...
    fn finished(&self, code: i32) {
        ...
        // THE WRONG DIRECTION — the security reviewer, 2026-09-04, race 2 of the deadlock
...
///
/// IT IS CREATED, AND IT HAS REAL FILES IN IT — the user's instruction: "a folder,
...
///
/// STRUCTURE ONLY, NEVER CONTENT — the same rule `a background memory indexer` already
/// banked for the product's memory graph, which this panel is the
...
/// memory-indexer build only ever fed a poll on a private box.
///
...
/// rule, and the same one `a background memory indexer` already uses.
...
/// and answers the user's actual question ("does Windows need anything") with a
...
/// Pick FILES to hand to the model. the user, 2026-08-27: "on the place to type we
...
fn main() {
    ...
    // SEND-SAFETY (the security reviewer re-review residual, 2026-09-09): the consequential
    ...
    builder
        ...
        // CLOSING THE WINDOW LEFT NameOS.exe RUNNING -- found
        // 2026-08-31: window destroyed, every WebView2 child gone, the
        // process itself still alive 187 seconds later. It happened to the user
...
mod tests {
    ...
    /// which is precisely what froze the user's window. Asserting on a return
    ...
    // -- The two races the security reviewer found in the deadlock fix -------------------
    ...
    /// CHILD — the security reviewer, 2026-09-04.** The `take()` rewrite that closed the
    ...
    /// the same orphan bug (`claude.exe` alive after every window is
        ...
        *session.turn.lock().unwrap() =
        ...
        assert!(
            ...
            "window-close's cancel_running_turn returned at {close_returned_at:?}, BEFORE the \
             ...
             would have raced an in-flight kill, which is the same orphan bug under a race"
        );
    }

    /// **RACE 2a: A LATE `finished()` CALL MUST NEVER WIPE A NEWER TURN OUT
    /// OF THE SLOT — the security reviewer, 2026-09-04.** While a turn's own `cancel()`
    ...
    /// TURN EITHER — the security reviewer, 2026-09-04, the other half of race 2.** The
    ...
    //
    // Everything below is the regression for the user, 2026-09-05, running the
        ...
        for (n, line) in body.lines().enumerate() {
            ...
            assert!(
                ...
                "AppSink line {n} calls a plain, unscoped .emit(...) -- that \
                 ...
                 exactly as reported on 2026-09-03: {}",
                line.trim()
            );
        }
    ...
    /// THE ACTUAL FAILURE, REPRODUCED, NOT ASSUMED AWAY -- found 2026-08-29.
    ...
    /// Windows binary it will actually wrap (`claude.exe`) is the release reviewer's to check.
    #[cfg(unix)]
    #[test]
    ...
    //
    // Pinned against the EXACT text the release reviewer captured from the real claude.exe
    ...
    fn the_real_notice_line_is_recognised() {
        ...
        // real binary, byte-verified by the release reviewer 2026-08-31 — this is the shape
        ...
        // it, and nothing here can confirm which one the user's own copy is.
