...
//!
//! The user asked for four things and they are all one subsystem: a GREEN INDICATOR
...
///
/// **THIS IS THE END-USER PATH.** the user, 2026-08-26: *"we are setting this up
...
/// worked.** the security reviewer found the first on 2026-08-28; the second fell out of
/// checking her prediction on the user's own Windows box the same day.
///
...
/// On the user's machine (Windows 10.0.26200), cmd given an unquoted URL cut it at
...
///
/// **THIS EXISTS BECAUSE THE LIST KEPT VANISHING.** the user, 2026-08-26: "Items
...
/// the stored credentials. the user, 2026-08-26: "There is no disconnect button
...
///
/// Pulled out of `claude_mcp_auth` on 2026-08-27 for one reason: **the bug the user
...
pub fn claude_mcp_auth(name: String, sign_in: bool) -> Result<(), String> {
    ...
    if out.status.success() {
        ...
        // The user, 2026-08-27: "the disconnect button does not work. I tried it on
...
///
/// The user, 2026-08-26: *"have a button to allow for it to disconnect"*. Distinct
...
fn build_launch_config(list: &[Connector]) -> Option<(String, Vec<(String, String)>)> {
    ...
    for c in list.iter() {
        ...
        // being an identifier. A person may well call a connector "the user's Mail".
...
///
/// **THE SCAR, 2026-08-26.** the user: "attaching email did not work." Every tile
...
mod list_tests {
    ...
    // THE DISCONNECT BUG, 2026-08-27 — the user: "the disconnect button does not
