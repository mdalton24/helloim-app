...
//! question, so they get more than a single opinion.
//! `the app spec`'s flagship feature, and the design doc
//! `AI-BOARDROOM-how-it-works.md` is the design this file exists to
//! implement. Read that file's own "The flow" section before changing the
//! shape below — this is a transcription of it, not a fresh design.
//!
...
//! error, ts }] }, synthesis: { seat, name, text, ts } | null`. The field
//! names `seat`/`name`/`text`/`error`/`ts` match, verbatim, the internal
//! multi-agent review format this design was modelled on — with `role` and
//! `stance` added, because a Boardroom seat carries a Primary/Secondary
//! designation that format does not.
//!
...
//!    first" mode (see `ui/index.html`'s `SAFE_OPTS`, sourced from the release reviewer's
...
pub struct SeatAnswer {
    /// The provider's own id — the internal review format's `seat` field,
    /// renamed for `serde`'s camelCase convention but the same key the TODO
    /// comment beside `#iaViewBoardroom` names.
    pub seat: String,
    pub name: String,
    /// "primary" | "secondary" | "" — never shown as a role the internal
    /// review format this was modelled on never had reason to carry.
    pub role: String,
...
impl TurnSink for CollectingSink {
    ...
    fn raw(&self, _line: String) {
        // Non-JSON stdout noise. This result shape has nowhere to
