//! The safe agency layer's three actions — v1, `the safety spec`, the
//! the design review's own amendments folded in 2026-09-04. Everything a
...
//! `the safety spec`'s own line on this). `OpenUrl` is the one action
...
//! `Start-Process`, the same injection-safe primitive the security reviewer hardened
...
//!
//! **TWO ESCAPES the security reviewer PROVED AGAINST THE FIRST VERSION OF THIS GUARD,
//! 2026-09-04 — both closed here, against the round-27 design already
//! agreed, not a re-decision.** `the safety spec`'s "the security reviewer ESCAPES"
...
//! calc.exe` against. The release reviewer's own runtime pass needs to prove all four
//! actually launch something recognisable, per `the safety spec`'s
//! build-order note.
//!
...
/// **CLOSED VOCABULARY, NO NORMALISATION — the safety spec, explicit.**
...
/// matches (`the safety spec`, explicit): `"Notepad "` with a trailing
...
/// as `is_unsafe_ipv6`.
///
/// **THE SIX RANGES the security reviewer PROVED STILL REACHABLE, 2026-09-04 — a browser
...
fn is_unsafe_ipv6(v6: &std::net::Ipv6Addr) -> bool {
    ...
    // **BOTH IPv4-EMBEDDING FORMS, ONE CHECK — the security reviewer's proven bypass,
...
/// **the security reviewer's PROVEN BYPASS #1 — anything that LOOKS like an attempt to
...
///
/// **REFUSED OUTRIGHT, NEVER RE-INTERPRETED.** The fix the security reviewer's own
...
/// on what it means.
///
/// **EVERY LABEL, NOT ANY ONE OF THEM — the security reviewer's second finding on this
...
/// **THE LOCKED ROUND-27 REQUIREMENT — DNS resolution, the security reviewer's second
...
/// **THE NETWORK-TARGET GUARD — the safety spec room amendment 1.** See
...
fn refuse_unsafe_target(url: &str) -> Result<(), String> {
    ...
    // **CANONICALISE BEFORE JUDGING — the security reviewer's proven bypass #1,
    ...
    // **the security reviewer's PROVEN BYPASS #1.** `ip_text` did not parse as a real
    ...
    // **the security reviewer's PROVEN BYPASS #2, THE LOCKED ROUND-27 REQUIREMENT.** The
...
pub(crate) fn launch_app(args: &Value) -> ToolResult {
    ...
    let Some((_, target)) = APPS.iter().find(|(name, _)| *name == requested) else {
        // **DELIBERATELY GENERIC — the safety spec: "refusal msg must not
...
mod tests {
    ...
    fn open_url_refuses_loopback_private_link_local_and_unspecified_ipv4() {
        for host in [
            "127.0.0.1", "127.5.5.5", "192.0.2.1", "172.16.0.1", "172.31.255.255",
    ...
    // -- OpenUrl: the six IPv4 ranges the security reviewer proved were still ALLOWED,
    ...
    // -- OpenUrl: the security reviewer's proven bypass #1, canonicalisation
    ...
    // -- The security reviewer's SECOND finding on this same guard, 2026-09-04: a
    ...
    /// The three of the security reviewer's five examples that actually resolve
    ...
    fn open_url_refuses_the_v4_compatible_ipv6_form_of_loopback() {
        ...
        // PROVEN by the security reviewer as "suspected" and confirmed here directly:
    ...
    // -- OpenUrl: the security reviewer's proven bypass #2, DNS resolution (2026-09-04),
