...
//! collided.
//!
//! **THE COLLISION.** the security reviewer proved, against this app's own spawn, that a
...
//!     rules *are* applied — which is exactly how the security reviewer's `getent` ran.
...
pub struct FolderTrust {
    ...
    pub has_bridge: bool,
    /// `.claude/settings.json` or `.claude/settings.local.json`. The security reviewer's
    ...
    /// The release reviewer's release verification, 2026-08-29, found a Windows machine whose
...
pub(crate) fn confine(root: &Path, requested: &str) -> Result<PathBuf, String> {
    ...
    loop {
        match std::fs::canonicalize(&probe) {
            ...
            Err(error) => {
                // The security reviewer's dangling-link write, 2026-09-07: canonicalize
...
pub(crate) fn display_name(raw: &str) -> Option<String> {
    ...
    for c in name.chars() {
        ...
        // Marie-Christine, A.L.E.X. Everything else — every format
...
fn read_claimed_name(nameos: &Path) -> Result<Option<String>, String> {
    ...
    // A BYTE-ORDER MARK MADE A VALID MANIFEST LOOK LIKE A MISSING ONE — the release reviewer's
...
/// The security reviewer's 9,005-byte proof on 2026-08-28. What this module adds is the
...
mod tests {
    ...
    /// The security reviewer's first proof: a `CLAUDE.md` obeyed on turn one with no gate.
    #[test]
    ...
    /// The security reviewer's second proof, and the reason this file exists in the shape
    ...
    /// planted one is exactly what the security reviewer walked in through.
    #[test]
    ...
    fn an_accepted_name_is_returned_unchanged() {
        for good in ["Bella", "O'Neill", "Marie-Christine", "A.L.E.X.", "Ada 2"] {
