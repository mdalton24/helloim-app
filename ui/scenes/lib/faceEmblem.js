// faceEmblem.js — the Giles face scene's centre mark, live on canvas.
//
...
//
// USED ONLY BY scenes/giles.js (the ported face scene). This app already
// has its OWN, unrelated emblem renderer inside scenes/board.js
// (drawEmblem, driven by hover/hoverable state this simpler mark does not
// have) — the two are deliberately separate files with no shared state,
// because they draw two different things: board.js's is that scene's own
// reactive mark, this is the simpler non-reactive DOOR-EMBLEM form also
// used on /login and /account. Do not merge them.
...
function paintEmblem(ctx, cx, cy, r, opts) {
  ...
  // explicitly; a hardcoded default name was a latent violation of the rule
  // that no fixed assistant name ever reaches canvas (board.js took the same
  // fix, swapping a hardcoded wordmark for the live assistant name).
