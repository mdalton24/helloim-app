...
/// helloim.ai/api/feedback is bound by the deploy pipeline to the helloim-api
...
fn send(
    ...
    match res {
        ...
        Err(ureq::Error::Status(503, _)) => {
            Err("Feedback is briefly unavailable. Try once more in a minute, or email hello@helloim.ai.".to_string())
        }
        Err(ureq::Error::Status(code, _)) => {
            Err(format!("helloim.ai answered {code}. Try again shortly, or email hello@helloim.ai."))
        }
