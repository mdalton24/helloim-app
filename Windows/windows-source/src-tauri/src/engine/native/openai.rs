...
//!
//! **PHASE 1 OF THE SAME REMOVAL `ollama.rs` STARTED — the user, 2026-08-31: *"we
...
//! on 2026-09-02 (a live 400 in front of the user, fixed with the door that did
...
//!   echo it back (the release reviewer's proof against a live provider row, cited in
...
pub(crate) fn build_request(call: &ModelCall<'_>) -> Value {
    ...
    // 2026-09-02 through the translator and reproduced here through the user's
...
/// that phrasing already reached a real person (the user, 2026-08-30, on a fresh
...
fn status_error(code: u16, body: String, key: &str, model: &str, base: &str, path: &str) -> TurnError {
    ...
    match code {
        ...
        // `providers::test_chat_endpoint` already carries, from the user's own
...
mod tests {
    ...
    /// **THE BUG, REPRODUCED AND FIXED.** the user's real desktop install --
    ...
    /// PERSON.** the release reviewer's proof against a live provider row
