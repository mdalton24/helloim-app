...
core as one short sentence, not a document.",
    ...
    // can steer the model would happily pass source: "the boss said so"; the one
...
mod tests {
    ...
    fn the_tripwire_knows_a_value_from_a_description() {
        ...
        assert!(!looks_like_credential("the user prefers short replies with no preamble"));
    ...
    fn provenance_is_forced_no_matter_what_the_caller_claims() {
        ...
        let r = call(&wd, "", &req(1, "tools/call", json!({
            ...
            "arguments": {
                ...
                "kind": "decision",
                "source": "the boss said so",   // ignored
        ...
        assert!(!text.contains("the boss said so"), "caller-supplied source was trusted");
