...
//! outside.
//!
//! **WIRED BACK UP 2026-08-29 AND SWITCHED ON THE SAME DAY.** the user: *"The user
...
//!
//! **WHY THIS EXISTS — the user's decision, 2026-08-27, overruling the earlier
...
//!
//! **THE SERVER IS NOT A WEB SERVER AND MUST NEVER GROW INTO ONE — an
//! explicit ruling on approval.** It speaks to exactly one known client, the `claude`
...
fn relay(stream: &mut TcpStream, base_url: &str, key: &str, mut body: Value) {
    ...
    let upstream = match response {
        ...
        Err(ureq::Error::Status(code, resp)) => {
            ...
            // The engineer, 2026-08-31, reusing `providers::redact_and_truncate`
            ...
            // kind of upstream — the release reviewer proved a live provider row echoing 23
...
mod tests {
    ...
    /// THE BUG: the user connected a real OpenAI key today, pointed a row at
    ...
    // -- The stream, including THE test that was specifically asked for. ---
    ...
    //
    // Testing proved this panic on an earlier shipped binary six times, from its
    ...
    // assumption the release reviewer disproved on the OTHER call site to the same kind of
    // upstream.
    //
    ...
    // loopback that answers 400 with a body built the same way the release reviewer's was —
    ...
    fn relay_redacts_the_key_and_never_panics_on_a_multibyte_boundary() {
        ...
        // A body shaped like the release reviewer's, with BOTH of the two things
    ...
    // (The user's own `openai-api` credential, decrypted straight into the
    ...
    /// The user's own `gemini-api` credential decrypted into this process only.
