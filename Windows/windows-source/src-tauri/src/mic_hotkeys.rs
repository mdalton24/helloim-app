...
// batch #2, ranked by the design review as the highest-value new work for
...
// another app, an IME eating it — is the release reviewer's to confirm on the real Windows
...
// recording` BELOW ARE THE FIX. The front-end reviewer traced the user's "hotkey listens but
...
// The release reviewer's real test (Win32 `SendInput`, a genuine OS-level keystroke — CDP
...
// call ends up running on. The release reviewer's `SendInput` re-test against a real build
...
/// end to end is the release reviewer's to confirm on the real box.
///
...
/// **THE SUSPEND/RESUME PAIR — 2026-09-05, item 3's missing half.** the front-end reviewer
/// traced the user's "hotkey listens but doesn't set the press" report to the
...
/// own comment on this pairing specifies for both), so a future caller can
...
/// above.** the release reviewer's test found THIS half already stable even as a plain
...
mod tests {
    ...
    /// this module (see the header) and is the release reviewer's to confirm.
    #[test]
