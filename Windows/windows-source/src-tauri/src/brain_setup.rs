...
//!
//! The user, 2026-08-27: *"The installer does not allow for options for what local
...
//! `notes` saying what could not be read and why, in the same discipline
//! system reporting works under: report what IS, and always say what you
//! could not determine.
//!
...
pub const CATALOG: &[Model] = &[
    ...
    // HERMES — the user, 2026-08-27: "hermes agent is popular. Why is it not on
...
fn run(cmd: &str, args: &[&str]) -> Option<String> {
    ...
    // The user, 2026-08-27, about the REMOTE probe: "it flashed a few times" — six
    ...
    // time the brain-setup wizard runs — the exact flow the user later reported as
...
//
// The user, 2026-08-27: *"Perhaps ask to run local or remote. If remote, ask for ip
...
//
// **READ-ONLY, BY CONSTRUCTION.** the user's instruction for the remote path was
...
/// Measured 2026-08-27 over SSH to 192.0.2.20: `nvidia-smi` answers perfectly,
...
/// 192.0.2.10: "It flashed a few times. No prompt for password. and then that
...
///
/// The user chose "password once, then install a key" for this flow, and that
...
pub fn key_hint(user: &str, host: &str, port: u16) -> String {
    ...
    // **NAME THE MACHINE, NOT "THIS COMPUTER" — the user, 2026-08-27, and it cost
...
///
/// **THE FAULT THIS FIXES, found by running it against 192.0.2.20:** a failing
    ...
    F: Fn(&str) -> Result<String, String>,
{
    ...
    // many words — measured against 192.0.2.20, where a missing binary exits
...
/// on a password nobody can see. The password bootstrap the user asked for -- give
...
pub fn ssh_run(host: &str, user: &str, port: u16, cmd: &str) -> Result<String, String> {
    ...
    // **THE FLASHING WINDOWS — the user, 2026-08-27: "It flashed a few times."**
...
//
// The user, 2026-08-28: *"Claude is not in the recommended to setup. The
...
// mentioned.
//
// **IT WAS SIX AND SIX FOR ONE DAY. the user, 2026-08-28, later the same day:
...
// sections teach is the distinction the user cares about most, which is whether
...
pub const CLOUD_AI: &[AiOption] = &[
    ...
    AiOption { id: "claude", name: "Claude", letter: "C", colour: "#d97757",
        ...
        download_url: "" },
    // OPENAI, BACK ON THE SCREEN 2026-08-29 — the user asked for this choice five
    ...
    // The user asked whether helloim.ai could connect to "each ai a user uses"
    ...
    // by the user's locked decision — and it is not needed here, because the
...
// The user named, and it is the one helloim.ai can actually measure a machine for.
//
...
mod tests {
    ...
    fn a_missing_command_is_not_a_refused_login() {
        ...
        assert!(is_unreachable("ssh: connect to host 192.0.2.9 port 22: No route to host"));
    ...
    fn the_recommended_screen_offers_one_and_one_and_leads_with_claude() {
        // IT WAS SIX AND SIX FOR ONE DAY. the user, 2026-08-28: "take out all
        ...
        assert_eq!(LOCAL_AI.len(), 1, "Ollama, the local path the user named");
