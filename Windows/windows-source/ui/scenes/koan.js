...
/**
 * KOAN — a luminous particle sphere on near-black.
 ...
 *
 * Rendered as a projected point cloud on 2D canvas (no WebGL dependency) with
 * an aurora wash behind it standing in for the background shader pass.
 */
const CANVAS_BG = '#0b0d10';
const DEFAULT_ACCENT = '#52ffa5';
let ACCENT = DEFAULT_ACCENT;
/* THE APPEARANCE OVERRIDE — a later design review, index.html's dock (see
 * noir.js's own, longer comment on this same mechanism for the full
 * reasoning and why board.js/capcom.js were deliberately excluded). HONEST
 * LIMIT, DISCLOSED RATHER THAN HIDDEN: ACCENT here only drives the
 * listening halo ring (see draw()'s `st.listening` branch below) — the
 * sphere's own dominant colour is computed per-point from a warm/cool mix
 * keyed to each point's position (`warm`, `rr/gg/bb` a few lines into
 * draw()), not from this constant, so an Appearance change visibly recolours
 * the ring but not the sphere itself. Wiring the sphere's own gradient to an
 * arbitrary accent is real, further work this pass did not attempt — see
 * this app's handback report. `let`, not `const`, because this is the one
 * property setPalette() below actually reassigns; CANVAS_BG stays const and
 * untouched for the same depth/contrast reason noir.js's ground tones do. */
function applyKoanPalette(colors) { ACCENT = (colors && colors.accent) || DEFAULT_ACCENT; }
        ...
        return {
            ...
            draw: (st) => {
                ...
                for (let i = 0; i < pts.length; i++) {
                    const p = pts[i];
                    const wob = Math.sin(p.v * 6 + st.t * 1.6 + p.u * 2) * 0.02 * churn +
                ...
                if (st.ping > 0.01) {
                    glow(st, cx, cy, R * (1 + st.ping * 2.4), st.flareSign < 0 ? 'rgba(255,90,90,0.5)' : 'rgba(140,255,210,0.5)', st.ping * 0.7);
                }
            },
        };
    });
