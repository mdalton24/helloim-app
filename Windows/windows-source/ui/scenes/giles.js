// giles.js — GILES theme scene: the HUD ring and its emblem, alone.
//
// CHANGED 2026-08-31 — the user, having seen 0.2.7 running on Windows: "on
// giles, remove background keep circle." He'd been carrying the ring
...
//
// Ported originally from helloim-v4's src/scenes/giles.ts — see
// renderer/lib/scene.js's header for the full port note. This file has now
// diverged from that source (the marketing site's full-width giles scene
...
export function mount(canvas, name) {
  ...
  // may put a word (least of all GILES) on the canvas.
  return createScene(canvas, (s) => {
    /* THE CENTREPIECE IS THE ACTUAL EMBLEM, not a silkscreened name — the user:
       "for the new application that we have for the theme that's named
       Giles, can you just put your center logo in that?"
