// THE GILES SCENE — a living circuit board, drawn from scratch on a single
// 2D canvas: chip, traces, packets and a chip die at the centre, all
// procedural.
//
...
//
// A few things worth knowing about the shape of this file: (1) the canvas
...
// chrome instead of owning it; (2) the board's decorative filler slots are
// replaced with THIS app's real connectors, via window.NameOSConnectorLEDs()
// — placed with the app's own rail/panel clearances, plus one added status
// dot so a connector's proven/unproven state (the same four colours the
// Apps sheet's own rows use) can never be invisible; (3) the die wordmark
// reads window.NameOSAssistantName() and scrambles through glyphs while
// thinking, fitted to width regardless of how long the name turns out to
// be. Hover on the emblem changes the cursor and its glow; nothing here
// responds to a click.
export function mount(canvas) {
  ...
  /* ===========================================================================
     This scene — living circuit board
     ---------------------------------------------------------------------------
  ...
  // --idle. This function and the four-colour convention it reads are
  ...
  function buildBoard() {
    ...
    /* This app shares its screen with a chat panel. Below the size a chat
       panel actually needs, the chip is dropped rather than half-covered —
       the state it carries still shows in the app's own top pill. */
    hasChip = (W >= 900 && H >= 620);
    ...
    /* THE COMPONENTS ARE THE PERSON'S ACTUAL CONNECTORS, not decorative
       filler and not synthetic IC packages. Same source and the same
       three-state rule the Apps sheet's own dots use
       (window.NameOSConnectorLEDs). With none connected the field is traces
       and vias and no packages — the honest empty state, not a decorative
       one. Placed with this app's own rail/panel clearances; drawn below by
       drawComps()'s own ic-package renderer, plus one added status dot
       (see drawComps). */
  ...
  function drawComps(c, vis) {
    ...
    for (const cp of comps) {
      ...
      if (cp.ic) {
        ...
        /* This app's connectors carry per-component status, and it must
           never be able to look proven when it is not (idle/warn/bad all
           read as visibly NOT ok). Same three-fact dot the Apps sheet's own
           connector rows use, read live so a theme switch or a fresh
           connection is never a stale colour baked in. */
        if (cp.c) {
          c.globalAlpha = 1;
  ...
  function step(dt) {
    ...
    // flare(false) below is a discrete event, so it holds alertTarget
    // at 1 for a short window (alertHold) rather than forever, then lets the
    // ease() curve (fast attack, slower release) bring it back down.
  ...
  /* ---- render ---------------------------------------------------------------
     Draws the baked board, then every live layer on top: bloom, pressure
     waves, packets, arrivals, and the chip's own die/emblem/indicators. */
  function render() {
    ...
    ctx.save();
    ctx.setTransform(DPR, 0, 0, DPR, 0, 0);   // world-space transform for this frame
    ctx.globalCompositeOperation = 'lighter';
  ...
  /* wordmarkFor() turns the assistant's real name into a fitted wordmark:
     dots for short names, plain for medium ones, initials past that, so it
     can never overrun the emblem regardless of what the name turns out to
     be. */
  ...
  /* HOVER ON THE EMBLEM — the cursor changing as well as the light is the
     convention people already know for "this does something". Hover-only;
  ...
  /* --- resize ----------------------------------------------------------------
     Uses canvas.getBoundingClientRect() rather than window.innerWidth/
     innerHeight, because this canvas shares the screen with the app's own
     chrome, same as every other scene here. */
  function resize() {
